...
input[type="email"] {
  ...
  border: 1px solid #d5d2cb; border-radius: 8px; background: #FAFAF8;
  color: #1a1a1a; transition: border-color 0.15s, background-color 0.15s;
  font-family: inherit;
}
input[type="email"]:focus { outline: none; border-color: #BA7517; box-shadow: 0 0 0 3px rgba(186,117,23,0.12); }
input[type="email"].email-error { border-color: #E24B4A; background: #FEF2F2; }
.email-hint { font-size: 12px; color: #999; margin-top: 6px; }
.email-err-msg { font-size: 12px; color: #C0392B; margin-top: 6px; display: none; }
.email-err-msg.visible { display: block; }
...
<div class="container">
  ...
  <div class="survey-header">
    ...
    <p>Your honest feedback helps Amber understand your experience and strengthen our partnership. This survey is fully anonymous — all responses are managed exclusively by an independent third-party agency.</p>
    <div class="badges">
  ...
  <div id="form-wrapper">
    ...
    <div class="email-card">
      ...
      <input type="email" id="email-input" placeholder="Enter your registered email address" autocomplete="email" />
      <div class="email-hint">Used only to record your response — not shared or stored publicly.</div>
      <div class="email-err-msg" id="email-err-msg"></div>
    </div>
    ...
    <div id="survey-form">
      ...
      <div class="submit-area">
        <button class="btn-submit" id="submit-btn" disabled onclick="submitRespond()">Submit My Response</button>
        <div class="anon-note">
          <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="11" width="18" height="11" rx="2" ry="2"/><path d="M7 11V7a5 5 0 0 1 10 0v4"/></svg>
          Responses are confidential and managed exclusively by an independent third-party agency.
        </div>
      </div>
  ...
  <div class="completion-box" id="completion">
    ...
    <p>Your response has been successfully recorded. All findings will be compiled by the third-party agency and shared with Amber's leadership team to drive meaningful improvements in our supplier relationships.</p>
...
const JS = `
  ...
  function handleSelect(sel) {
    ...
    const answered = document.querySelectorAll('#survey-form select.answered').length;
  ...
  function showError(msg) {
    const errEl = document.getElementById('email-err-msg');
    const inputEl = document.getElementById('email-input');
    if (errEl) { errEl.textContent = msg; errEl.classList.add('visible'); }
    if (inputEl) { inputEl.classList.add('email-error'); }
  ...
  function clearError() {
    ...
    if (inputEl) { inputEl.classList.remove('email-error'); }
  }
  ...
  async function submitRespond() {
    ...
    if (!email || !email.includes('@')) {
      showError('Please enter a valid email address before submitting.');
      return;
    }
    ...
    const selects = Array.from(document.querySelectorAll('#survey-form select'));
    ...
    remarks.forEach(function(ta, i) {
      payload['q' + (i + 1) + '_remark'] = ta.value.trim() || '';
    });

    try {
      const res = await fetch('/api/submit-respond', {
        method: 'POST',
      ...
      if (res.status === 403) {
        ...
        showError('This email address is not registered for this survey. Please check your email or contact the survey coordinator.');
        return;
      }
      ...
      if (res.status === 409) {
        btn.disabled = false;
        btn.textContent = 'Submit My Response';
        showError('A response has already been submitted for this email address. Thank you for your participation.');
        return;
      }
